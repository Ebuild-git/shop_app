    ...
    public function form_update_prix()
    {
        ...
        if ($post) {
            ...
            if ($lastChange && $lastChange->created_at >= $oneWeekAgo) {
                ...
                // Only allow price reductions (compare raw prices)
                if ($newRawPrix > $old_price) {
                ...
                // Save the raw price (model's getPrix() will add percentage on display)
                $post->prix = $newRawPrix;
                $post->updated_price_at = now();
                $post->save();

                // Record history using the raw values
                $history = new History_change_price();
                $history->id_post = $post->id;
                $history->old_price = $old_price;
                $history->new_price = $newRawPrix;
                $history->save();
