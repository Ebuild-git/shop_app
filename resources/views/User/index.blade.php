                            ...
                            <div class="card-body p-0">
                                <div class="shop_thumb position-relative">
                                    ...
                                    <div class="badge-container position-absolute top-0 start-0 d-flex gap-4 mobile-display-luxe" style="z-index: 5;">
                                        ...
                                        @if($lux->sell_at && !$lux->hasDeletedOrders())
                            ...
                            <div class="card-body p-0">
                                <div class="shop_thumb position-relative">
                                    ...
                                    <div class="badge-container position-absolute top-0 start-0 d-flex gap-4 mobile-display" style="z-index: 5;">
                                        ...
                                        @if($last->sell_at && !$last->hasDeletedOrders())
                                            <div class="badge-new badge-sale bg-danger text-white">
                                                {{ \App\Traits\TranslateTrait::TranslateText('Vendu') }}
                                            </div>
                                        @endif
...
@endsection --}}
@extends('User.fixe')
@section('titre', 'Accueil - Shopin Luxury')
@section('body')
    ...
    <div class="position-relative">
        <div class="carousel">
            @forelse ($categories_carousel as $cat)
                <div class="carousel-item text-center" style="background-image: url('{{ Storage::url($cat->icon) }}');">
                    <a href="{{ route('shop', ['id_categorie' => $cat->id]) }}" class="carousel-btn" aria-label="{{ $cat->titre }}">
                        @php
                            $locale = app()->getLocale();
                            switch ($locale) {
                                case 'en':
                        ...
                        @if ($cat->luxury)
                            <i class="bi bi-gem gem-icon" aria-hidden="true"></i>
                        @endif
                    </a>
                </div>
        ...
        <div class="group-btn-slide">
            <button class="btn-slide-home" onclick="prevItem()" aria-label="Précédent">
                <i class="bi bi-arrow-left" aria-hidden="true"></i>
            </button>
            <button class="btn-slide-home" onclick="nextItem()" aria-label="Suivant">
    ...
    <section class="middle">
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                    <div class="sec_title position-relative text-center">
                        <h1 class="ft-bold pt-3">{!! __('nouveau_sur_shopin_luxury') !!} {!! __('luxury2') !!}</h1>
                    </div>
                </div>
            </div>
            <div class="row align-items-center">
                        ...
                        <div class="product_grid card b-0">
                            <div class="card-body p-0">
                                <div class="shop_thumb position-relative">
                                    ...
                                    <div class="badge-container position-absolute top-0 start-0 d-flex gap-4 mobile-display-luxe" style="z-index: 5;">
                                        @if($lux->discountPercentage)
                                            <div class="badge-new badge-discount" aria-label="Réduction {{ $lux->discountPercentage }}%">
                                                -{{ $lux->discountPercentage }}%
                                            </div>
                                        @endif

                                        @if($lux->sell_at && !$lux->hasDeletedOrders())
                                            <div class="badge-new badge-sale bg-danger text-white" aria-label="Produit vendu">
                                                {{ \App\Traits\TranslateTrait::TranslateText('Vendu') }}
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    <button type="button" class="badge badge-like-post-count btn-favorite-post position-absolute ab-right cusor {{ $lux->isFavoritedByUser(Auth::id()) ? 'active' : '' }}"
                                        id="post-{{ $lux->id }}" data-post-id="{{ $lux->id }}"
                                        onclick="toggleFavorite({{ $lux->id }})"
                                        aria-label="{{ $lux->isFavoritedByUser(Auth::id()) ? 'Retirer des favoris' : 'Ajouter aux favoris' }}"
                                        aria-pressed="{{ $lux->isFavoritedByUser(Auth::id()) ? 'true' : 'false' }}">
                                    <i class="bi bi-suit-heart-fill" aria-hidden="true"></i>
                                    <span class="count">{{ $lux->favoris->count() }}</span>
                                    </button>

                                    <!-- Product Image -->
                                    <a class="card-img-top d-block overflow-hidden" href="/post/{{ $lux->id }}" aria-label="Voir {{ $lux->titre }}">
                                        <img src="{{ Storage::url($lux->photos[0] ?? '') }}" alt="{{ $lux->titre }}" loading="lazy">
                                    </a>
                                </div>
                            </div>
                            <!-- SubCardPost Component -->
            ...
            @if ($luxurys->count() > 0)
                <div class="row justify-content-center">
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                        <div class="position-relative text-center">
                            <a href="/shop?luxury_only=true" class="btn stretched-link borders" aria-label="{{ \App\Traits\TranslateTrait::TranslateText('Voir Plus de produits luxe') }}">
                                <span class="voir-plus-home-texte">
                                    {{ \App\Traits\TranslateTrait::TranslateText('Voir Plus') }}
                                </span>
                                <i class="lni lni-arrow-right ml-2" aria-hidden="true"></i>
                            </a>
                        </div>
                    </div>
                </div>
    ...
    <section class="middle">
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                    <div class="sec_title position-relative text-center">
                        <h2 class="ft-bold pt-3">{!! __('nouveau_sur_shopin_luxury') !!}</h2>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-xl-3 col-lg-4 col-md-6 col-6">
                        <div class="product_grid card b-0">
                            ...
                            <button type="button" class="badge badge-like-post-count btn-favorite-post position-absolute ab-right cusor {{ $last->isFavoritedByUser(Auth::id()) ? 'active' : '' }}"
                                id="post-{{ $last->id }}" data-post-id="{{ $last->id }}"
                                onclick="toggleFavorite({{ $last->id }})"
                                aria-label="{{ $last->isFavoritedByUser(Auth::id()) ? 'Retirer des favoris' : 'Ajouter aux favoris' }}"
                                aria-pressed="{{ $last->isFavoritedByUser(Auth::id()) ? 'true' : 'false' }}">
                            <i class="bi bi-suit-heart-fill" aria-hidden="true"></i>
                            <span class="count">{{ $last->favoris->count() }}</span>
                            </button>
                            ...
                            <div class="card-body p-0">
                                <div class="shop_thumb position-relative">
                                    ...
                                    <div class="badge-container position-absolute top-0 start-0 d-flex gap-4 mobile-display" style="z-index: 5;">
                                        @if($last->discountPercentage)
                                            <div class="badge-new badge-discount" aria-label="Réduction {{ $last->discountPercentage }}%">
                                                -{{ $last->discountPercentage }}%
                                            </div>
                                        @endif
                                    ...
                                    <!-- Product Image -->
                                    <a class="card-img-top d-block overflow-hidden" href="/post/{{ $last->id }}" aria-label="Voir {{ $last->titre }}">
                                        <img src="{{ Storage::url($last->photos[0] ?? '') }}" alt="{{ $last->titre }}" loading="lazy">
                                    </a>
                                </div>
                            </div>
                            <!-- SubCardPost Component -->
            ...
            <div class="row justify-content-center">
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                    <div class="position-relative text-center">
                        <a href="/shop" class="btn stretched-link borders" aria-label="{{ \App\Traits\TranslateTrait::TranslateText('Voir Plus de produits') }}">
                            <span class="voir-plus-home-texte">
                                {{ \App\Traits\TranslateTrait::TranslateText('Voir Plus') }}
                            </span>
                            <i class="lni lni-arrow-right ml-2" aria-hidden="true"></i>
